...
class LazopClient
{
	...
	{
	    $key_config = \Config::get('lazada');
	    if(!isset($key_config['connections']) || !isset($key_config['connections']['app']))
	    {
            throw new \Exception("app_key or app_secret is empty",0);
        }
		$this->gatewayUrl = empty($url) ? $key_config['connections']['app']['url'] : $url;
		$this->appkey = $key_config['connections']['app']['app_key'];
		$this->secretKey = $key_config['connections']['app']['app_secret'];
...
//			$this->logApiError($requestUrl, $respObject->code, $respObject->message);
            $this->logCommunicationError('', $requestUrl, "HTTP_ERROR_" . $respObject->code, $respObject->message);
//            $result         = [];
//            $result['code'] = $respObject->code;
//            $result['msg']  = $respObject->message;
...
//				$this->logApiError($requestUrl, '', '');
                $this->logCommunicationError('', $requestUrl, '', '');
                $result         = [];
//                $result['code'] = '';
//                $result['msg']  = '';
                return json_decode(json_encode($respObject),true);
    ...
    protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt)
    {
        $localIp                   = isset($_SERVER["SERVER_ADDR"]) ? $_SERVER["SERVER_ADDR"] : "CLI";
        $logger                    = new LazopLogger();
        $logger->conf["log_file"]  = rtrim($this->logPath, '\\/') . '/' . "logs/lazada/lazada_comm_err_" . $this->appKey . "_" . date("Y-m-d") . ".log";
        $logger->conf["separator"] = "^_^";
        $logData                   = array(
            date("Y-m-d H:i:s"),
            $apiName,
